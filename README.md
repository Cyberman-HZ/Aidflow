...
| **✈️ Drone Camp Planner — 🚧 Beta** — upload an aerial / drone image of the site; Gemma 4 vision identifies tents, water points, latrines, paths, open areas, buildings, and vehicles into a structured layout, with each detection carrying a high / medium / low confidence that drives marker opacity and weights downstream population math. An interactive canvas overlays the detections; an **Edit mode** lets the admin click to delete, add, or relocate features the model missed (AI proposes, admin verifies — same pattern as function calling). A right-rail panel turns the layout into nine operational planning tasks: population estimate (tent count × household size, confidence-weighted), suggested distribution-point centroid, suggested medical-tent centroid (weighted by family vulnerability), Sphere ratios with gap callouts, route hints, underserved-zones heatmap, unregistered-arrivals delta, evacuation readiness, and a flood / hazard polygon painter that flags at-risk tents. **Time-series snapshots** keep every upload as a separate row; pick a baseline in the *Compare to* dropdown to see new tents ringed in green, departed tents as dashed red ghosts, and moved tents as amber arrows, alongside Δ-population and tents-per-day growth metrics. Family pins bind individual families to specific tent positions. ([`campMap.ts`](src/services/campMap.ts), [`CampMap.tsx`](src/pages/CampMap.tsx)) | Planning that previously took a paid GIS contractor weeks now happens in 30 seconds on the field laptop — and a *second* upload a week later visualizes how the camp has grown without any manual bookkeeping. The coordinator decides where to put the next distribution point, where to drill the next latrine, and which tents to evacuate first — all from one drone snapshot, all offline, all explainable via the same Trace button as the rest of the AI. **Why beta:** Gemma 4 E4B has a real ceiling on small-object localization in aerial imagery. The Edit mode is the principled fix today (and demos honestly as "AI proposes, human decides"); **the roadmap is to fine-tune Gemma 4 on labeled aerial camp imagery via [Unsloth](https://unsloth.ai/) QLoRA** to raise the no-edit accuracy floor before v1.0 — see Section 5 below. |
...
│       └── test-camp-map-shape.mjs            ← Drone Camp Planner contract: types, v11 + v12 migrations, service exports, Sphere constants, live pointInPolygon + diffSnapshots sanity tests, Edit-mode wiring, time-series + diff overlay contract, /camp-map route + nav + locale (124 assertions)
│
└── src/
    ...
    │   ├── CampMapCanvas.tsx           ← Drone Camp Planner canvas: image + SVG overlay of detected features (confidence-aware opacity), View / Pin / Edit / Hazard modes with brush picker, and compare-snapshot diff overlay (green ring = new tents, dashed red = removed, amber arrows = moved)
    │   └── CampMapInsights.tsx         ← Drone Camp Planner right-rail panel: 9 operational tasks + snapshot-comparison delta card (kept / moved / added / removed, Δ pop, growth-rate)
    │
    ├── db/
    │   ├── database.ts                 ← Dexie schema + migrations (v1 through v12)
    ...
    │   ├── campMap.ts                  ← Drone Camp Planner: Gemma 4 vision call (top-left-origin prompt + worked example + dedup) + JSON sanitizer + Sphere-ratio + geometry helpers (pointInPolygon, weighted centroids) + multi-snapshot CRUD (listCampMaps / getCampMap / deleteCampMap) + diffSnapshots(active, compare) for time-series comparison
...
## 🔮 5. What's next — Android companion + fine-tuned vision model

The hackathon entry is the coordinator-side console. Two roadmap pillars take this further:
**(a)** carrying the same architecture down to **the field worker's phone, mid-tent-visit**, and
**(b)** lifting the Drone Camp Planner's accuracy floor with a **fine-tuned Gemma 4 vision adapter** so the Edit-mode safety net becomes optional rather than necessary.

### 🤖 5a. Android companion app
...
### 🛰️ 5b. Fine-tuning the Drone Camp Planner vision model (Unsloth QLoRA)

The Drone Camp Planner ships as **Beta** because Gemma 4 E4B (~8 B) has a real ceiling on **small-object localization in aerial imagery**. A typical refugee-camp drone shot at 1600 px gives the model ~10–30 pixels per tent — at the edge of what a general-purpose VLM can reliably count and place. The Edit mode in v0.x is the honest fix today (AI proposes, admin verifies). The roadmap to a stronger no-edit floor is targeted fine-tuning:

| Pillar | Plan |
|---|---|
| **Dataset** | 500–2,000 labeled aerial / satellite images of IDP / refugee settlements with normalized-coordinate annotations for every tent, water point, latrine, building, and path. Source candidates: [HOTOSM Tasking Manager](https://tasks.hotosm.org/) shelter-mapping projects, the [xView](https://challenge.xviewdataset.org/) humanitarian subset, [UNOSAT](https://unitar.org/maps) imagery, public drone footage, plus in-house labeling via [Roboflow](https://roboflow.com/) / [CVAT](https://www.cvat.ai/). |
| **Training** | **[Unsloth](https://unsloth.ai/) QLoRA** on Gemma 4 E4B's multimodal stack — 2–5× faster than vanilla HF, runs on a single consumer GPU (4090) or rented A100 hour. LoRA adapters keep the base weights untouched so the same fine-tune can be merged or shipped as an overlay. |
| **Schema target** | Fine-tune the model to emit AidFlow Pro's exact JSON schema (the one in `campMap.ts`'s `SYSTEM_PROMPT`) with calibrated `high` / `medium` / `low` confidence — collapsing the "stay inside the contract" failure mode that even strong VLMs hit on small-image batches. |
| **Deployment** | Export the LoRA → GGUF → serve via the existing Ollama pipeline. **No change to the privacy contract** — inference still runs on `localhost:11434` and bytes never leave the laptop. Adapter published on Hugging Face under MIT so other humanitarian projects can reuse it. |
| **Evaluation** | Extend `scripts/qa/test-camp-map-shape.mjs` with a per-image expected-counts harness, run base-vs-fine-tuned A/B against a held-out set, and track precision / recall on each feature type plus mean coordinate error per tent. |

The realistic bottleneck is **data acquisition**, not training compute. A 2–3 week sprint after the hackathon — most of it labeling — is enough for a measurably better v1.0 release. Until then, the Edit mode keeps the feature honest for life-safety contexts.
