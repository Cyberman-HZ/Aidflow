// Knowledge Base — merged page combining the PDF document library and
// the aid usage guides. Sub-tabs let users flip between them; the AI
// Q&A panel on the right answers questions across both sources
// (PDFs via RAG, guides via the system prompt context).
...
import type { KnowledgeDocument, AidGuide } from '@/types';
    ...
    <div className="space-y-5">
      ...
      <div className="flex border-b border-slate-700 -mt-1">
        <TabButton
          ...
          label={`${t('knowledge.tab_documents')} (${docs.length})`}
        />
        <TabButton
          active={tab === 'guides'}
          onClick={() => setTab('guides')}
          icon={<HelpCircle size={14} />}
          label={`${t('knowledge.tab_guides')} (${guides.length})`}
        />
      </div>

      <div className="grid lg:grid-cols-[1fr_400px] gap-5">
        <div className="space-y-3">
          {tab === 'documents' ? <DocumentsTab docs={docs} /> : <GuidesTab guides={guides} />}
        </div>
...
function DocumentsTab({ docs }: { docs: KnowledgeDocument[] }) {
  const { t } = useTranslation();
          ...
          <div className="mt-4 bg-ai/10 border border-ai/30 rounded-lg p-3 text-sm">
            ...
            <div className="mt-2 h-1 bg-surface-light rounded overflow-hidden">
              <div
                ...
                style={{
                  width: phase === 'extract' ? '33%' : phase === 'embed' ? '66%' : '100%',
                }}
              />
            </div>
          </div>
...
function GuidesTab({ guides }: { guides: AidGuide[] }) {
  const { t } = useTranslation();
  ...
  return (
    <>
      ...
      {filtered.length === 0 ? (
        <Card>
          <EmptyState icon={<HelpCircle size={28} />} title={t('guides.no_guides')} />
        </Card>
      ) : (
        filtered.map((g) => (
          <article
